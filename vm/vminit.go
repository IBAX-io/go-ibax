...
const (
	...
	// VMTypeSmart is langcode vm type
...
var (
	RegisterAutoPars = map[string]string{
		//`*smart.Contract`:      `sc`,
...
var ContractPrices = map[string]string{
	`@1NewTable`: `price_create_table`, `@1NewContract`: `price_create_contract`,
	`@1NewEcosystem`: `price_create_ecosystem`, `@1NewMenu`: `price_create_menu`,
	`@1NewPage`: `price_create_page`, `@1NewColumn`: `price_create_column`,
...
func ExecContract(rt *RunTime, name, txs string, params ...interface{}) (interface{}, error) {
	...
	if priceName, ok := ContractPrices[name]; ok {
		price := store.SysInt64(priceName)
		if price > 0 {
			rt.cost -= price
		}
		if rt.cost < 0 {
