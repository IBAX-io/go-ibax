...
func VDEScheTaskChainStatus(ctx context.Context, d *daemon) error {
	var (
		err             error
	...
	for _, item := range ScheTask {
		...
		ScheTaskChainStatusSrc := model.VDEScheTaskChainStatus{
		...
		if err = ScheTaskChainStatusSrc.Create(); err != nil {
			log.WithFields(log.Fields{"error": err}).Error("Insert vde_sche_task_chain_status table failed")
			src_chainstatus_flag = 0
			time.Sleep(time.Millisecond * 100)
			continue
		}
		fmt.Println("Insert vde_sche_task_chain_status table ok")
		...
		for index, vde_dest_pubkey_item := range vde_dest_pubkey_slice {
			...
			if err = ScheTaskChainStatusDest.Create(); err != nil {
				log.WithFields(log.Fields{"error": err}).Error("Insert vde_sche_task_chain_status table failed")
				dest_chainstatus_flag = 0
				time.Sleep(time.Millisecond * 100)
				continue
			}
			fmt.Println("Insert vde_sche_task_chain_status table ok:", index)
		} //for
