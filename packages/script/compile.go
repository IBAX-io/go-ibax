		...
		case lexIdent:
			...
			if i < len(*lexems)-2 {
				if (*lexems)[i+1].Type == isLPar {
					...
					if isContract {
						name := StateName((*block)[0].Owner.StateID, lexem.Value.(string))
					...
					if lexem.Value.(string) == `CallContract` {
						count++
						bytecode.push(newByteCode(cmdPush, lexem.Line, (*block)[0].Owner.StateID))
					}
					parcount = append(parcount, count)
					call = true
				}
				if (*lexems)[i+1].Type == isLBrack {
					...
					buffer.push(newByteCode(cmdIndex, lexem.Line, &IndexInfo{VarOffset: objInfo.GetInt(), Owner: tobj}))
				}
			}
			if !call {
