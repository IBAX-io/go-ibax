...
func (btc *BlockTimeCounter) NodeTimeExists(t time.Time, nodePosition int) (bool, error) {
	...
	//	return false, err
	//}
	fmt.Println("nodeps:", nodePosition, "btcps:", ps, "t:", t.Unix())
	//fmt.Println("nodeps:",nodePosition,"btcps",ps)
...
func NewBlockTimeCounter() *BlockTimeCounter {
	firstBlock, _ := store.GetFirstBlockData()
	blockGenerationDuration := time.Millisecond * time.Duration(store.GetMaxBlockGenerationTime())
	blocksGapDuration := time.Second * time.Duration(store.GetGapsBetweenBlocks())
	btc := BlockTimeCounter{
		start:       time.Unix(int64(firstBlock.Time), 0),
		duration:    blockGenerationDuration + blocksGapDuration,
		numberNodes: int(store.GetCountOfActiveNodes()),
	}
