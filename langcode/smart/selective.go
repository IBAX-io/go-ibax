...
func (sc *SmartContract) selectiveLoggingAndUpd(fields []string, ivalues []interface{},
	...
	if generalRollback {
		...
		//if sqlBuilder.Table != "1_delayed_contracts"{
		//	off := strings.IndexByte(sqlBuilder.Table, '_')
		//	if sc.TxSmart.EcosystemID == converter.StrToInt64(sqlBuilder.Table[:off]) {
		//		fmt.Println(fmt.Sprintf("BlockID:%d, TableName:%s, TableID:%s",sc.BlockData.BlockID, sqlBuilder.Table, sqlBuilder.TableID()))
		//	}
		//}
		var tid string
