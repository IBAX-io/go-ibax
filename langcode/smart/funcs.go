...
import (
	...
	"ibax.io/conf"
	qb "ibax.io/langcode/smart/queryBuilder"
	"ibax.io/miner"
	"ibax.io/obsmanager"
	"ibax.io/scheduler"
	"ibax.io/scheduler/contract"
	"ibax.io/store"
	"ibax.io/store/model"
	"ibax.io/store/tx"
...
type FlushInfo struct {
	ID   uint32    // id
	Prev *vm.Block // previous item, nil if the new item has been appended
	Info *vm.ObjInfo
...
// SmartContract is storing langcode contract data
type SmartContract struct {
	...
	NewElementPricesFuel decimal.Decimal
...
func EmbedFuncs(v *vm.VM, vt vm.VMType) {
	...
	case vm.VMTypeOBS:
		...
		vmExtendCost(v, getCost)
		vmFuncCallsDB(v, funcCallsDB)
	case vm.VMTypeOBSMaster:
		f["HTTPRequest"] = HTTPRequest
		f["GetMapKeys"] = GetMapKeys
		f["SortedKeys"] = SortedKeys
		f["Date"] = Date
...
func CompileContract(sc *SmartContract, code string, state, id, token int64) (interface{}, error) {
	...
	return VMCompileBlock(sc.VM, code, &vm.OwnerInfo{StateID: uint32(state), WalletID: id, TokenID: token})
}
...
func contractName(value string) (name string, err error) {
	...
	list, err = vm.ContractsList(value)
...
func ValidateEditContractNewValue(sc *SmartContract, newValue, oldValue string) error {
	list, err := vm.ContractsList(newValue)
	if err != nil {
		return err
	}
	curlist, err := vm.ContractsList(oldValue)
	if err != nil {
		return err
	}
...
// CreateTable is creating langcode contract table
...
func FlushContract(sc *SmartContract, iroot interface{}, id int64) error {
	...
	root := iroot.(*vm.Block)
	if id != 0 {
		if len(root.Children) != 1 || root.Children[0].Type != vm.ObjContract {
			return errOneContract
		}
	}
	for i, item := range root.Children {
		if item.Type == vm.ObjContract {
			root.Children[i].Info.(*vm.ContractInfo).Owner.TableID = id
		}
	}
	for key, item := range root.Objects {
		if cur, ok := sc.VM.Objects[key]; ok {
			...
			switch item.Type {
			case vm.ObjContract:
				id = cur.Value.(*vm.Block).Info.(*vm.ContractInfo).ID
			case vm.ObjFunc:
				id = cur.Value.(*vm.Block).Info.(*vm.FuncInfo).ID
			}
...
func columnConditions(sc *SmartContract, columns string) (err error) {
	...
	if len(cols) > store.GetMaxColumns() {
		return logErrorfShort(eManyColumns, store.GetMaxColumns(), consts.ParameterExceeded)
	}
	for _, icol := range cols {
		var data map[string]interface{}
		switch v := icol.(type) {
...
func ColumnCondition(sc *SmartContract, tableName, name, coltype, permissions string) error {
	...
	if count >= int64(store.GetMaxColumns()) {
		return logErrorfShort(eManyColumns, store.GetMaxColumns(), consts.ParameterExceeded)
	}
...
func UpdateNodesBan(smartContract *SmartContract, timestamp int64) error {
	...
	banRequests, err := badBlocks.GetNeedToBanNodes(now, store.GetIncorrectBlocksPerDay())
	if err != nil {
	...
	for i, fullNode := range fullNodes {
		...
		for _, banReq := range banRequests {
			if banReq.ProducerNodeId == nodeKeyID && banReq.Count >= int64((len(fullNodes)/2)+1) {
				fullNode.UnbanTime = now.Add(store.GetNodeBanTime())
				...
				banMessage := fmt.Sprintf(
					...
					banReq.Count,
					len(fullNodes),
					store.GetIncorrectBlocksPerDay(),
				)
				...
				_, _, err = DBInsert(
					...
					types.LoadMap(map[string]interface{}{
						...
						"ban_time":  int64(store.GetNodeBanTime() / time.Millisecond), // in ms
						"reason":    banMessage,
					}))
	...
	if updFullNodes {
		...
		_, err = UpdateSysParam(smartContract, store.FullNodes, string(data), "")
		if err != nil {
...
func TransactionData(blockId int64, hash []byte) (data *TxInfo, err error) {
	...
	for blockBuffer.Len() > 0 {
		...
		if bytes.Equal(txHash, hash) {
			...
			txInfo := contract.Block.Info.(*vm.ContractInfo).Tx
			if txInfo != nil {
...
func Throw(code, errText string) error {
	if len(errText) > vm.MaxErrLen {
		errText = errText[:vm.MaxErrLen] + `...`
	}
	if len(code) > 32 {
		code = code[:32]
	}
...
func SendExternalTransaction(sc *SmartContract, uid, url, externalContract string,
	...
	if _, err := store.GetThisNodePosition(); err != nil {
		return nil
	}
...
func IsFullNodeKey(id int64) bool {
	for _, item := range store.GetNodes() {
