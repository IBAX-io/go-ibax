...
func SysRollbackContract(name string, EcosystemID int64) error {
	v := GetVM()
	if c := VMGetContract(v, name, uint32(EcosystemID)); c != nil {
		id := c.Block.Info.(*vm.ContractInfo).ID
		if int(id) != len(v.Children)-1 {
			err := fmt.Errorf(eRollbackContract, id, len(v.Children)-1)
			log.WithFields(log.Fields{"type": consts.VMError, "error": err}).Error("rollback contract")
			return err
		}
		v.Children = v.Children[:id]
		delete(v.Objects, c.Name)
	}
...
func SysRollbackNewContract(sysData SysRollData, EcosystemID string) error {
	contractList, err := vm.ContractsList(sysData.Data)
	if err != nil {
		return err
	}
...
func SysFlushContract(iroot interface{}, id int64, active bool) error {
	...
	for i, item := range root.Children {
		if item.Type == vm.ObjContract {
			...
			root.Children[i].Info.(*vm.ContractInfo).Owner.Active = active
		}
	}
...
func SysSetContractWallet(tblid, state int64, wallet int64) error {
	for i, item := range smartVM.Block.Children {
		if item != nil && item.Type == vm.ObjContract {
			cinfo := item.Info.(*vm.ContractInfo)
			if cinfo.Owner.TableID == tblid && cinfo.Owner.StateID == uint32(state) {
				smartVM.Children[i].Info.(*vm.ContractInfo).Owner.WalletID = wallet
			}
		}
	}
	return nil
...
func SysRollbackEditContract(transaction *model.DbTransaction, sysData SysRollData,
	...
	if len(fields["value"]) > 0 {
		...
		root, err := VMCompileBlock(GetVM(), fields["value"],
			&vm.OwnerInfo{StateID: uint32(owner.StateID), WalletID: wallet, TokenID: owner.TokenID})
		if err != nil {
...
func SysRollbackEcosystem(DbTransaction *model.DbTransaction, sysData SysRollData) error {
	...
	if sysData.ID == 1 {
		...
		v := GetVM()
		for v.Children[len(v.Children)-1].Type == vm.ObjContract {
			cinfo := v.Children[len(v.Children)-1].Info.(*vm.ContractInfo)
