...
func getCost(name string) int64 {
	if price, ok := store.GetPriceExec(utils.ToSnakeCase(name)); ok {
		return price
	}
	return -1
}
...
func UpdateSysParam(sc *SmartContract, name, value, conditions string) (int64, error) {
	...
	check:
		switch name {
		case store.GapsBetweenBlocks:
			ok = ival > 0 && ival < 86400
		case store.RbBlocks1, store.NumberNodes:
			ok = ival > 0 && ival < 1000
		case store.CommissionSize, store.PriceCreateRate, store.PriceTxSizeWallet, store.BlockReward:
			ok = ival >= 0
		case store.MaxBlockSize, store.MaxTxSize, store.MaxTxCount, store.MaxColumns,
			store.MaxIndexes, store.MaxBlockUserTx, store.MaxTxFuel, store.MaxBlockFuel, store.MaxForsignSize:
			ok = ival > 0
		case store.FuelRate, store.CommissionWallet:
			...
			for _, item := range list {
				...
				case store.FuelRate, store.CommissionWallet:
					if len(item) != 2 || converter.StrToInt64(item[0]) <= 0 ||
						(name == store.FuelRate && converter.StrToInt64(item[1]) <= 0) ||
						(name == store.CommissionWallet && converter.StrToInt64(item[1]) == 0) {
						break check
					}
				}
			}
			checked = true
		case store.FullNodes:
			fnodes := []*conf.FullNode{}
			if err := json.Unmarshal([]byte(value), &fnodes); err != nil {
				break check
			}
			if len(fnodes) > 1 {
				if err = conf.DuplicateFullNode(fnodes); err != nil {
	...
	err = store.SysUpdate(sc.DbTransaction)
	if err != nil {
...
func MoneyDiv(d1, d2 interface{}) string {
	val1, _ := vm.ValueToDecimal(d1)
	val2, _ := vm.ValueToDecimal(d2)
...
func LangRes(sc *SmartContract, idRes string) string {
	ret, _ := language.LangText(idRes, int(sc.TxSmart.EcosystemID), sc.TxSmart.Lang)
	return ret
}
...
func GetContractByName(sc *SmartContract, name string) int64 {
	...
	info := (*contract).Block.Info.(*vm.ContractInfo)
	if info == nil {
		return 0
	}
...
func CreateEcosystem(sc *SmartContract, wallet int64, name string) (int64, error) {
	...
	if _, _, err = DBInsert(sc, "@1parameters", types.LoadMap(map[string]interface{}{
		"name": "ecosystem_wallet", "value": "0", "conditions": `ContractConditions("@1AdminCondition")`,
		"ecosystem": idStr,
	})); err != nil {
...
//TimeDeltaDays
func TimeDeltaDays(timeStamp int64, days string) int64 {
	if timeStamp == 0 {
